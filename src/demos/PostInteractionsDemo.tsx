...
window.fetch = ((url: string, options?: RequestInit) => {
  if (typeof url === "string" && url.includes("/like")) {
    return Promise.resolve(new Response(JSON.stringify({ likes_count: 43 }), { status: 200 }));
  }
  if (typeof url === "string" && url.includes("/rate")) {
    return Promise.resolve(new Response(JSON.stringify({ rating_total: 554, rating_count: 128 }), { status: 200 }));
  }
...
export function PostInteractionsDemo() {
  return (
    <div className="max-w-2xl mx-auto px-6 py-8 space-y-10">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Post Interactions</h1>
      ...
      {/* PostLikes */}
      <section>
        <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Post Likes</h2>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
          <div className="flex items-center gap-8 flex-wrap">
            ...
            <div>
              ...
              <PostLikes postId="demo-2" likesCount={128} apiBase="/api" size="sm" />
            </div>
            <div>
      ...
      {/* PostRating */}
      <section>
        <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Post Rating</h2>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 flex flex-col gap-6">
          <div className="flex items-start gap-10 flex-wrap">
            <div>
              <p className="text-xs text-gray-400 mb-2">Com votos (4.3 ★)</p>
              <PostRating postId="demo-r1" postTitle="Demo Post" postUrl="https://example.com" ratingTotal={549} ratingCount={127} apiBase="/api" />
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-2">Sem votos</p>
              <PostRating postId="demo-r2" postTitle="Novo Post" postUrl="https://example.com" ratingTotal={0} ratingCount={0} apiBase="/api" />
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-2">Small (sm)</p>
              <PostRating postId="demo-r3" postTitle="Demo Post" postUrl="https://example.com" ratingTotal={549} ratingCount={127} apiBase="/api" size="sm" />
            </div>
          </div>
        </div>
        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
